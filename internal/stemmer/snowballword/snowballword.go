...
// --- Rune-optimized variants (zero-allocation suffix matching) ---

// FirstSuffixRunes returns the first matching suffix rune slice.
func (w *SnowballWord) FirstSuffixRunes(suffixes ...[]rune) []rune {
	return w.FirstSuffixIfInRunes(0, len(w.RS), suffixes...)
}

// FirstSuffixIfInRunes checks startPos constraint and returns the first match.
func (w *SnowballWord) FirstSuffixIfInRunes(startPos, endPos int, suffixes ...[]rune) []rune {
	for _, suffixRunes := range suffixes {
		if w.HasSuffixRunesIn(0, endPos, suffixRunes) {
			if endPos-len(suffixRunes) >= startPos {
				return suffixRunes
			}
...
// FirstSuffixInRunes searches within a region and returns the first match.
func (w *SnowballWord) FirstSuffixInRunes(startPos, endPos int, suffixes ...[]rune) []rune {
	for _, suffixRunes := range suffixes {
		if w.HasSuffixRunesIn(startPos, endPos, suffixRunes) {
			return suffixRunes
		}
...
// RemoveFirstSuffixIfInRunes removes the first suffix found after startPos.
func (w *SnowballWord) RemoveFirstSuffixIfInRunes(startPos int, suffixes ...[]rune) []rune {
	suffixRunes := w.FirstSuffixIfInRunes(startPos, len(w.RS), suffixes...)
	if suffixRunes != nil {
		w.RemoveLastNRunes(len(suffixRunes))
	}
	return suffixRunes
...
// RemoveFirstSuffixInRunes removes the first suffix found in the region.
func (w *SnowballWord) RemoveFirstSuffixInRunes(startPos int, suffixes ...[]rune) []rune {
	suffixRunes := w.FirstSuffixInRunes(startPos, len(w.RS), suffixes...)
	if suffixRunes != nil {
		w.RemoveLastNRunes(len(suffixRunes))
	}
	return suffixRunes
